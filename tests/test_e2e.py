"""E2E & integration tests for deep-report v0.2.

Test categories:
- v0.2 P2 features (credibility table, risk evolution, peer formatting)
- Pipeline integration (mocked LLM, real data flow)
- Cross-market validation (A-share, HK, US)
"""
...
def _make_kpi_entry(period: str, **kwargs) -> dict:
    ...
    entry = {
        ...
        "business_model": kwargs.pop("business_model", {
            "segments": [
                {"name": "主品牌", "revenue": 4500, "revenue_unit": "百万元",
                 "revenue_yoy": "+25.0%", "revenue_share_pct": 79.0,
                 "gross_margin": 43.5},
            ],
        }),
        "one_time_items": kwargs.pop("one_time_items", [
            {"name": "投资收益", "value": 880, "unit": "百万元",
             "nature": "non-recurring", "description": "AI投资公允价值变动"},
        ]),
        "cash_flow_quality": kwargs.pop("cash_flow_quality", {
            "operating_cash_flow": 1200, "ocf_unit": "百万元",
            "net_profit_match": {"ocf_to_net_profit_ratio": 0.85, "assessment": "healthy"},
            "free_cash_flow": 800, "fcf_unit": "百万元",
            "capex": 400, "capex_unit": "百万元",
        }),
...
class TestPipelineIntegration:
    ...
    def test_format_kpis_includes_business_model(self):
        from deep_report.analyzer import ReportAnalyzer
        kpis = [_make_kpi_entry("2026Q1")]
        result = ReportAnalyzer()._format_kpis_for_llm(kpis)
        assert "业务模型数据" in result
        assert "主品牌" in result
        assert "79.0%" in result

    def test_format_kpis_includes_one_time_items(self):
        from deep_report.analyzer import ReportAnalyzer
        kpis = [_make_kpi_entry("2026Q1")]
        result = ReportAnalyzer._format_kpis_for_llm(None, kpis)
        assert "一次性/非经常性项目" in result
        assert "投资收益" in result

    def test_format_kpis_includes_cash_flow_quality(self):
        from deep_report.analyzer import ReportAnalyzer
        kpis = [_make_kpi_entry("2026Q1")]
        result = ReportAnalyzer._format_kpis_for_llm(None, kpis)
        assert "现金流质量" in result
        assert "OCF/净利润比值" in result

    def test_format_kpis_includes_guidance(self):
        from deep_report.analyzer import ReportAnalyzer
        kpis = [_make_kpi_entry("2026Q1")]
        result = ReportAnalyzer._format_kpis_for_llm(None, kpis)
        assert "管理层指引" in result
        assert "海外门店加速扩张" in result
    ...
    def test_full_pipeline_passing_validation(self, monkeypatch, tmp_path):
        """When validation passes, pipeline should produce narrative."""
        from deep_report.analyzer import ReportAnalyzer
        from deep_report.writer import ReportWriter

        extract_count = [0]  # mutable counter

        def mock_call_llm(self, prompt, max_tokens=4000):
            extract_count[0] += 1
            if "提取所有可量化的关键指标" in prompt or "extract" in prompt.lower():
                return json.dumps({
                    "company_name": "名创优品",
                    "kpis": [
                        {"field": "revenue", "value": 5690, "unit": "百万元"},
                    ...
                    "management_guidance": {
                        "revenue_outlook": "下季增长25%",
                        "risk_mentions": ["汇率波动"],
                    },
                })
            # Analyze pass — return mock narrative
            return """### 🧭 核心观点
...
本季营收5690百万元，同比增长28.5%，超指引上限。"""

        monkeypatch.setattr(ReportAnalyzer, "_call_llm", mock_call_llm)

        # SDK returns matching values (no rejection)
        sdk_data = {
            "income_statement": {"revenue": {0: 5690000000}, "net_profit": {0: 550000000}},
            "balance_sheet": {"total_assets": {0: 10000000000}},
            "cash_flow": {},
        }
        monkeypatch.setattr(ReportAnalyzer, "_load_sdk_data", lambda self, c: sdk_data)

        analyzer = ReportAnalyzer(verify=True)
        monkeypatch.setattr(analyzer, "_extract_text", lambda p, m: "TEST REPORT TEXT")
        monkeypatch.setattr(analyzer, "_fetch_peer_data", lambda c: [])
        ...
        result = analyzer.analyze("MNSO", "2026Q1", reports)
        assert result is not None
        assert "_rejected" not in result
        assert result["narrative"] is not None
        assert "核心观点" in result["narrative"]
