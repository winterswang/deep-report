...
"""
    ...
    def test_overlap_dedup(self, analyzer):
        """Overlapping sections (>50% overlap) should be deduplicated"""
        # Pad text to >200 chars so sections are created (not fallback-triggered)
        padding = "x" * 200
...
""" + padding
        ...
        assert "income_statement" in result
        # Single anchor matched in this short text; dedup logic is tested
        # implicitly by en_market test (short report → balance_sheet may be deduped)
